...
<style>
  .modal-purple .modal-header{background:#5b57d1;color:#fff}
  .modal-purple .btn-close{filter:invert(1);opacity:.9}
  .btn-badge{padding:.25rem .5rem}
  .badge-pill{border-radius:999px;padding:.35rem .55rem;font-weight:600}
  .badge-closed{background:#16a34a}   /* green */
  .badge-posted{background:#9ca3af}   /* gray */
  .badge-in{background:#22c55e}       /* green */
  .badge-out{background:#ef4444}      /* red */
  .btn-detail{background:#3b82f6;border-color:#3b82f6}
  .btn-detail:hover{filter:brightness(.95)}
  .btn-export{background:#e5e7eb;border-color:#e5e7eb}
</style>
...
<div class="page-content">
      ...
      <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
          <div class="page-title-left">
            <h4 class="mb-0">History Stock FG — Index</h4>
          </div>
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              <li class="breadcrumb-item"><a href="javascript:void(0)">Master Data</a></li>
              <li class="breadcrumb-item active">List Stock FG</li>
            </ol>
          </div>
        </div>
      </div>
  ...
  @endif
          ...
          <div class="card-body">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2" id="custom-controls">
              ...
              <div class="input-group" style="max-width: 360px;">
                <button class="btn btn-light" type="button" data-bs-toggle="modal" data-bs-target="#filterMonthModal">
            ...
            <table class="table table-bordered table-hover dt-responsive w-100" id="ssTable" style="font-size: small">
              ...
              <tbody>
                @foreach($rows as $i => $r)
                  <tr>
                    {{-- # (nomor urut) --}}
                    <td class="align-middle text-center text-bold freeze-column">{{ $i + 1 }}</td>
                    ...
                    <td class="align-middle text-center">
                      @if($r->type_stock === 'IN')
                        <button type="button" class="btn btn-light btn-sm btn-badge btn-type-stock"
                                data-type="IN" data-bs-toggle="modal" data-bs-target="#infoStockModal">
                          <i class="mdi mdi-arrow-down-bold"></i>
                          <span class="badge badge-pill badge-in text-white">IN</span>
                        </button>
                      @elseif($r->type_stock === 'OUT')
                        <button type="button" class="btn btn-light btn-sm btn-badge btn-type-stock"
                                data-type="OUT" data-bs-toggle="modal" data-bs-target="#infoStockModal">
...
<script>
  $(function() {
    const dt = $('#ssTable').DataTable({
      ...
      order: [[6, 'desc']], // sort default by Date (kolom index 6)
      dom: 't<"d-flex justify-content-between align-items-center mt-2"lip>',
    });

    $('#lengthDT').on('change', function(){ dt.page.len(this.value).draw(); });
    $('#custom-search-input').on('keyup change', function(){ dt.search(this.value).draw(); });
...
<div class="modal fade" id="filterMonthModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
